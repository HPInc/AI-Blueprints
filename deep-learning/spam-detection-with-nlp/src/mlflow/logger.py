...
- Provides log_model functionality for spam detection models
- Handles artifact organization and temporary directory management
- Uses MLflow's models-from-code approach for deployment
- Manages configuration, spam dataset, NLTK data, and demo assets
"""
import os
import uuid
import base64
...
class Logger:
    ...
    This class provides the log_model functionality for packaging spam detection
    model with NLP preprocessing capabilities.
    """
    ...
    def log_model(
        ...
        """
        Log spam detection model using refined models-from-code approach.
        
        This implementation uses MLflow's models-from-code approach exclusively with proper
        temp directory naming for spam detection artifacts.
        ...
        /artifacts/
          └── data/                    # MLflow automatically created
              ...
              ├── nltk_data/           # NLTK stopwords and data
              └── demo/                # UI components  
        ...
        Args:
            ...
            data_file_path: Path to the spam dataset CSV file
            nltk_data_path: Path to the NLTK data directory
        ...
        try:
            ...
            # ✅ Copy spam dataset -> /artifacts/data/spam_utf8.csv
            if data_file_path and os.path.exists(data_file_path):
                shutil.copy2(data_file_path, os.path.join(temp_dir, "spam_utf8.csv"))
                logger.info(f"Copied spam dataset from {data_file_path}")
            else:
                logger.info("Spam dataset path not provided or doesn't exist - skipping")
            
            # ✅ Copy NLTK data -> /artifacts/data/nltk_data/
            if nltk_data_path and os.path.exists(nltk_data_path):
                shutil.copytree(nltk_data_path, os.path.join(temp_dir, "nltk_data"))
                logger.info(f"Copied NLTK data from {nltk_data_path}")
            else:
                logger.info("NLTK data path not provided or doesn't exist - skipping")
