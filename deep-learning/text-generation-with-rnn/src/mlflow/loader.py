...
def _load_pyfunc(data_path: str):
    ...
    if os.path.exists(data_file_path):
        with open(data_file_path, 'r', encoding='utf8') as f:
            text = f.read()
        all_chars = set(text)
        logger.info(f"Loaded character set from training data: {len(all_chars)} unique characters")
    else:
        # Fallback: load from encoder keys if data file not available
        import torch
        encoder_dict = torch.load(encoder_path)
        all_chars = set(encoder_dict.keys())
        logger.info(f"Loaded character set from encoder: {len(all_chars)} unique characters")
