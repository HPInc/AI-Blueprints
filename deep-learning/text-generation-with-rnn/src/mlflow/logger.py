    ...
    def log_model(
        ...
        try:
            ...
            if not os.path.exists(model_state_dict_path):
                raise FileNotFoundError(f"Model state dict not found at: {model_state_dict_path}")
            shutil.copy2(model_state_dict_path, os.path.join(temp_dir, "model_state_dict"))
