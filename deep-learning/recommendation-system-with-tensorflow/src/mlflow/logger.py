...
class Logger:
    ...
    def log_model(
        ...
        try:
            logger.info(f"Organizing movie recommendation artifacts in temp directory: {temp_dir}")
            ...
            if not os.path.exists(train_data_matrix_path):
                raise FileNotFoundError(f"Training data matrix not found at: {train_data_matrix_path}")
            shutil.copy2(train_data_matrix_path, os.path.join(temp_dir, "train_data_matrix.npy"))
