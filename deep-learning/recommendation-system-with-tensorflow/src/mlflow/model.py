...
class Model:
    ...
    def normalize_ratings(
        self, ratings: np.ndarray, min_rating: int = 1, max_rating: int = 5
    ) -> np.ndarray:
        """
        ...
        normalized_ratings = (ratings - ratings.min()) / (
    ...
    def predict(
        ...
        try:
            ...
            logger.info(
                f"Generating recommendations for movie_id: {movie_id}, rating: {rating}"
            )
            ...
            ordered_list = sorted(
                predictions_with_titles, key=lambda x: x[1], reverse=True
            )
