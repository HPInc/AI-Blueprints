"""
Utility functions for classification with SVM.

This module contains common functions used across notebooks in the project,
including configuration loading and model initialization.
"""

import os
import yaml
from typing import Dict, Any, Tuple


def load_config(config_path: str = "../configs/config.yaml") -> Dict[str, Any]:
    """
    Load configuration from YAML file.

    Args:
        config_path: Path to the configuration YAML file.

    Returns:
        Configuration dictionary.

    Raises:
        FileNotFoundError: If the config file is not found.
    """
    # Convert to absolute path if needed
    config_path = os.path.abspath(config_path)

    if not os.path.exists(config_path):
        raise FileNotFoundError(f"config.yaml file not found in path: {config_path}")

    with open(config_path) as file:
        config = yaml.safe_load(file)

    return config


def configure_proxy(config: Dict[str, Any]) -> None:
    """
    Configure proxy settings based on provided configuration.

    Args:
        config: Configuration dictionary that may contain a "proxy" key.
    """
    if "proxy" in config and config["proxy"]:
        os.environ["HTTPS_PROXY"] = config["proxy"]


def get_ui_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract UI configuration from the main config.
    
    Args:
        config: Main configuration dictionary.
        
    Returns:
        UI configuration dictionary with defaults if not specified.
    """
    ui_config = config.get("ui", {})
    
    # Set defaults
    defaults = {
        "mode": "streamlit"
    }
    
    # Merge with defaults
    for key, default_value in defaults.items():
        if key not in ui_config:
            ui_config[key] = default_value
    
    return ui_config


def get_port_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract port configuration from the main config.
    
    Args:
        config: Main configuration dictionary.
        
    Returns:
        Port configuration dictionary with defaults if not specified.
    """
    port_config = config.get("ports", {})
    
    # Set defaults
    defaults = {
        "external": 5000,
        "internal": {
            "gradio": 7860,
            "streamlit": 8501,
            "static": 5001
        }
    }
    
    # Merge with defaults
    for key, default_value in defaults.items():
        if key not in port_config:
            port_config[key] = default_value
    
    return port_config


def get_service_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract service configuration from the main config.
    
    Args:
        config: Main configuration dictionary.
        
    Returns:
        Service configuration dictionary with defaults if not specified.
    """
    service_config = config.get("service", {})
    
    # Set defaults
    defaults = {
        "mlflow_timeout": 600,
        "health_check_timeout": 300
    }
    
    # Merge with defaults
    for key, default_value in defaults.items():
        if key not in service_config:
            service_config[key] = default_value
    
    return service_config
