...
def split_text_if_too_large(
    text: str, max_size: int = LARGE_FILE_CHARACTER_LIMIT
) -> list[str]:
    """
    ...
    return split_text_if_too_large(part1, max_size) + split_text_if_too_large(
        part2, max_size
    )
        ...
        for item in tree_response.json().get("tree", []):
            ...
            try:
                content_response = requests.get(
                    content_url, headers=headers, timeout=self.timeout
                )
                ...
                print(
                    f"Warning: Timed out while fetching '{path}'. Skipping this file."
                )
                continue
            except Exception as e:
                print(
                    f"Warning: Could not process '{path}' due to an error: {e}. Skipping file."
                )
                continue
...
with mid_col:
    ...
    with tab2:
        uploaded_files = st.file_uploader(
            "Upload .md or .zip files", type=["md", "zip"], accept_multiple_files=True
        )
        if st.button(
                        ...
                        for filename in z.namelist():
                            if filename.endswith(".md") and not filename.startswith(
                                ...
                                with z.open(filename) as f:
                                    files_to_process[filename] = f.read().decode(
                                        "utf-8"
                                    )
                elif uploaded_file.name.endswith(".md"):
                    files_to_process[uploaded_file.name] = (
...
if files_to_process:
    with mid_col:
        ...
        with st.spinner(
            "Initializing the model... This can take a few minutes on the first run."
        ):
            try:
                # Use a string for the warmup payload to match the real calls
                warmup_df = pd.DataFrame(
                    [{"repo_url": None, "files": "This is a warmup."}]
                )
                warmup_payload = {"dataframe_split": warmup_df.to_dict("split")}
                # Check the response and raise an error if it fails
                requests.post(
                    api_url, json=warmup_payload, timeout=180, verify=False
                ).raise_for_status()
            except Exception as e:
                st.error(
                    f"Failed to initialize the model. Please check the API URL and ensure the backend is running correctly. Error: {e}"
                )
        ...
        for i, (filename, content) in enumerate(files_to_process.items()):
            progress_bar.progress(
                (i + 1) / total_files,
                text=f"Processing file {i+1} of {total_files}: {filename}",
            )

            # Split the file content only if it's an anomaly.
            # Most files will result in a list with a single item.
            pieces = split_text_if_too_large(
                content, max_size=LARGE_FILE_CHARACTER_LIMIT
            )
                    ...
                    try:
                        ...
                        if "corrected_file.md" in corrected_content_dict:
                            corrected_pieces.append(
                                corrected_content_dict["corrected_file.md"]
                            )
                        else:
                        ...
                        st.warning(
                            f"A part of {filename} failed to process: {e}. Keeping original content for this part."
                        )
                        corrected_pieces.append(piece)
                        continue
...
if st.session_state.corrected_files:
    with mid_col:
        ...
        all_metrics = {"total_processing_time": st.session_state["response_time"]}
            ...
            for i, (name, value) in enumerate(all_metrics.items()):
                if isinstance(value, (int, float)):
                    ...
                    metric_cols[i % 5].metric(
                        label=formatted_label, value=formatted_value
                    )
        ...
        if corrected_files:
            ...
            if selected_file:
                ...
                corrected_text_content = corrected_files.get(
                    selected_file, ""
                )  # Get current version

                s = difflib.SequenceMatcher(
                    None, original_text_lines, corrected_text_content.splitlines()
                )
                ...
                for i, group in enumerate(grouped_opcodes):
                    ...
                    for tag, i1, i2, j1, j2 in group:
                        ...
                        display_corrected.extend(
                            corrected_text_content.splitlines()[j1:j2]
                        )
