...
class QwenVLMM:
    ...
    def __init__(self, llm: LLM, tok: AutoTokenizer, image_processor: AutoImageProcessor, 
                 device: str, text_db: Chroma, image_db: Chroma, 
                 bm25_index: Optional[BM25Okapi], doc_map: dict):
        self.llm = llm
        self.tok = tok
    ...
    def _reciprocal_rank_fusion(results: list[list[Document]], k: int = 60) -> list[tuple[Document, float]]:
        ...
        fused_results = [(all_docs[content], rrf_scores[content]) 
    ...
    def _retrieve_mm(self, query: str, k_text: int = 3, k_img: int = 2, recall_k: int = 20) -> dict[str, any]:
        ...
        return {
            ...
            "text_sources": [doc.metadata.get("source", "Unknown") for doc in selected_docs],
            "image_sources": [hit.metadata.get("source", "Unknown") for hit in image_hits] if image_hits else []
        }
    ...
    def generate(self, query: str) -> dict[str, any]:
        ...
        referenced_sources = list(set(retrieval_results["text_sources"] + retrieval_results["image_sources"]))
...
class Model:
    ...
    def __init__(self, local_model_dir: str, e5_model_dir: str, siglip_model_dir: str, config: dict):
        """
        ...
        self.text_embed_model = HuggingFaceEmbeddings(
            model_name=e5_model_dir, 
            model_kwargs={"device": self.device}
        )
        
        logger.info("Loading image embedding model (SigLIP)...")
        self.siglip_embed_model = SiglipEmbeddings(model_id=siglip_model_dir, device=self.device)
        ...
        with self.db_lock:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                try:
                    ...
                    if self.judge:
                        context_str = "\n\n".join(
                            d.page_content for d in transient_kb["text_db"].similarity_search(query, k=3)
                        )
