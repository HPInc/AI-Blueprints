...
class Model:
    ...
    def __init__(self, model_no_finetuning_path: str, model_finetuning_path: str, config: Dict[str, Any] = None):
        """
    ...
    def _validate_model_paths(self):
        ...
        if finetuned_path.exists():
            ...
            if fp16_files:
                logger.info(f"📁 Found {len(fp16_files)} fp16 variant files in fine-tuned model")
            else:
                logger.warning("⚠️  No fp16 variant files found in fine-tuned model - will use standard loading")
        else:
    ...
    def _load_pipeline(self, use_finetuning: bool):
        ...
        logger.info(f"Loading {'fine-tuned' if use_finetuning else 'base'} model from: {mdl_path}")
    ...
    def _load_model_with_fallbacks(self, mdl_path: str, device: str, use_finetuning: bool):
        ...
        if torch.cuda.is_available():
            try:
                logger.info(f"Attempting to load {model_type} model with fp16 variant and dtype")
                ...
                if "variant=fp16" in str(e):
                    logger.warning(f"{model_type.title()} model lacks fp16 variant - this is common for fine-tuned models")
                else:
                    logger.warning(f"Failed to load {model_type} model with fp16 variant: {e}")
            except Exception as e:
                logger.warning(f"Unexpected error loading {model_type} model with fp16 variant: {e}")
        ...
        try:
            logger.info(f"Loading {model_type} model with fp32 dtype (compatibility mode)")
            return StableDiffusionPipeline.from_pretrained(
                mdl_path, 
        ...
        try:
            logger.warning(f"Loading {model_type} model with minimal configuration (last resort)")
            return StableDiffusionPipeline.from_pretrained(
                mdl_path,
                low_cpu_mem_usage=True
            ).to(device)
        except Exception as e:
            logger.error(f"Failed to load {model_type} model even with minimal configuration: {e}")
        ...
        try:
            ...
            if torch.cuda.is_available() and hasattr(pipeline, 'enable_sequential_cpu_offload'):
                ...
                if gpu_memory < 12:  # For GPUs with less than 12GB
                    ...
                    logger.info("Enabled sequential CPU offloading for memory management")
        except Exception as e:
            logger.warning("Could not enable CPU offloading: %s", e)

    def predict(self, model_input: Union[pd.DataFrame, dict], params: Dict[str, Any] = None) -> pd.DataFrame:
        """
