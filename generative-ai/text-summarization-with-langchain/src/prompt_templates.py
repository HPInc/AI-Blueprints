...
You are a helpful assistant that provides accurate and concise responses for text summarization.<|eot_id|><|start_header_id|>user<|end_header_id|>
...
def get_summarization_prompt(model_source: str = "local") -> PromptTemplate:
    ...
    formatted_template = get_prompt_template_for_model_source(
        model_source, SUMMARIZATION_TEMPLATE
    )
...
def get_chunk_summarization_prompt(model_source: str = "local") -> PromptTemplate:
    ...
    """
    formatted_template = get_prompt_template_for_model_source(
        model_source, CHUNK_SUMMARIZATION_TEMPLATE
    )
    return PromptTemplate.from_template(formatted_template)


def format_summarization_prompt(
    model_source: str, context_placeholder: str = "{context}"
) -> str:
    """
...
def format_chunk_summarization_prompt(
    model_source: str, context_placeholder: str = "{context}"
) -> str:
    """
...
def get_custom_summarization_prompt(
    model_source: str, custom_instructions: str, context_placeholder: str = "{context}"
) -> PromptTemplate:
    """
