...
class LLMComparisonModel(mlflow.pyfunc.PythonModel):
    def load_context(self, context):
        ...
        try:
            ...
            logging.info(
                f"📊 Using device: {self.device}, LoRA mode: {self.ft_is_lora}"
            )
            ...
            if use_ft:
                ...
                if ft_path.exists() and ft_path.is_dir():
                    ...
                    if has_full_weights and not is_lora:
                        ...
                        self.current_tokenizer = self.ft_tokenizer or _load_tokenizer(
                            ft_path
                        )
                        ...
                        base_tokenizer = self.base_tokenizer or _load_tokenizer(
                            self.base_src
                        )
                        ...
                        try:
                            ...
                            self.current_tokenizer = (
                                self.ft_tokenizer or _load_tokenizer(ft_path)
                            )
                            self.current_model = _load_model(
                                ft_path, device="auto"
                            ).eval()
                ...
                self.current_tokenizer = self.base_tokenizer or _load_tokenizer(
                    self.base_src
                )
