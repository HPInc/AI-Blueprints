"""
Utility functions for the Text Generation GenAI Blueprint.
...
def generate_code_with_retries(chain, example_input, callbacks=None, max_attempts=3, min_code_length=10):
    """
    Generate code with retries to handle potential failures.
    ...
    Returns:
        Generated and cleaned code string
    ...
    for attempt in range(max_attempts):
        try:
            ...
            # Validate the result
            if len(cleaned_code.strip()) >= min_code_length:
                logger.info(f"✅ Code generation successful on attempt {attempt + 1}")
                return cleaned_code
            else:
                logger.warning(f"⚠️  Generated code too short on attempt {attempt + 1}, retrying...")
                
        except Exception as e:
            logger.warning(f"⚠️  Code generation failed on attempt {attempt + 1}: {str(e)}")
