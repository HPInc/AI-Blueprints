...
from model_selection import ModelSelector # Model selection utility
...
class AudioState(TypedDict, total=False):
    question: str
    file_id: str
    index: Any  # not used here; we keep vecs/metas on self
    memory: Any
    audio_llm: Any
    is_relevant: bool
    from_memory: bool
...
class AudioAgenticPyFunc(mlflow.pyfunc.PythonModel):
    ...
    def load_context(self, context):
        ...
        vecs_name = os.environ.get("INDEX_VECS_NPY", "audio_vecs.npy")
        meta_name = os.environ.get("INDEX_META_JSON", "audio_meta.json")
        ...
        self.memory = SimpleKVMemory(memory_dir / mem_file)
        ...
        # --- Graph (same topology as your notebook’s build function) ---
        def node_ingest_question(state: AudioState) -> AudioState:
            msgs = state.get("messages", [])
            msgs.append({"role": "developer", "content": "Ingested question"})
            return {"messages": msgs}

        def _embed_text(q: str) -> np.ndarray:
            inp = self.clap_processor(text=[q], return_tensors="pt")
            out = self.clap_model.get_text_features(**inp)
        ...
        def node_check_relevance_audio(state: AudioState) -> AudioState:
            ...
            qvec = _embed_text(q)
            # quick relevance by max score
            raw = _dot_search(self.vecs, self.metas, qvec, k=max(self.fetch_k, self.top_k))
            max_score = max((h.get("score", 0.0) for h in raw), default=-1.0)
            is_rel = bool(max_score >= self.relevance_threshold)
            msgs = state.get("messages", [])
            msgs.append({"role": "developer", "content": f"Relevance: max_score={max_score:.3f}->{'relevant' if is_rel else 'irrelevant'}"})
            return {"is_relevant": is_rel, "hits_raw": raw, "messages": msgs}

        def node_check_memory(state: AudioState) -> AudioState:
            q = (state.get("question") or "").strip().lower()
            fid = state.get("file_id") or "global"
            key = f"{fid} :: {q}"
            cached = _mem_get(self.memory, key)
            if cached:
                return {
                    "from_memory": True,
        ...
        def node_retrieve(state: AudioState) -> AudioState:
            if state.get("from_memory"):
                return {}
            # we already computed hits_raw in relevance check
            raw = state.get("hits_raw", [])
            return {"hits_raw": raw}

        def node_rerank(state: AudioState) -> AudioState:
            if state.get("from_memory"):
                return {}
            hits = _mmr(state.get("hits_raw", []), top_k=self.top_k)
            return {"hits": hits}
        ...
        def node_generate_audio(state: AudioState) -> AudioState:
            ...
            for e in ev:
                e.setdefault("score", e.get("score_mmr", e.get("score", 0.0)))
            return {"answer": out.get("answer", ""), "evidence": ev}

        MEMOIZE_MIN_SCORE = float(os.environ.get("MEMOIZE_MIN_SCORE", "0.50"))
        def node_update_memory(state: AudioState) -> AudioState:
            if state.get("from_memory"):  # nothing to do
                return {}
            if not state.get("is_relevant"):
                return {}
            ev = state.get("evidence") or []
            best = max((e.get("score", 0.0) for e in ev), default=0.0)
            if best < MEMOIZE_MIN_SCORE or not ev or not state.get("answer"):
                return {}
            q = state["question"].strip().lower()
            fid = state.get("file_id", "global")
            key = f"{fid} :: {q}"
            _mem_put(self.memory, key, {"answer": state["answer"], "evidence": ev})
            return {}
