...
def convert_ffmpeg(src, dst, acodec=None, vcodec=None, extra=None):
    if not ffmpeg_ok():
        print("⚠️  ffmpeg not found; skipping", Path(dst).name); return
...
def generate_and_convert_formats(media_path: Path) -> Dict[str, str]:
    ...
    if ffmpeg_ok():
        if not AUDIO_TARGETS["mp3"].exists():
            convert_ffmpeg(MASTER_WAV, AUDIO_TARGETS["mp3"], acodec="libmp3lame", extra=["-b:a","160k"])
        if not AUDIO_TARGETS["m4a"].exists():
            convert_ffmpeg(MASTER_WAV, AUDIO_TARGETS["m4a"], acodec="aac", extra=["-b:a","160k"])
    else:
    ...
    for k in ["mp4", "mov", "mkv"]:
        if not VIDEO_TARGETS[k].exists():
            make_video_with_audio_ffmpeg(MASTER_WAV, VIDEO_TARGETS[k],
                                        vcodec="libx264", acodec="aac",
                                        size="1280x720", fps=24, bg_color="black")

    # AVI prefers MPEG-4 Part 2 + MP3
    if not VIDEO_TARGETS["avi"].exists():
        make_video_with_audio_ffmpeg(MASTER_WAV, VIDEO_TARGETS["avi"],
                                    vcodec="mpeg4", acodec="libmp3lame",
                                    size="1280x720", fps=24, bg_color="black",
                                    extra=["-qscale:v","3"])  # quality knob for mpeg4

    # WEBM (VP9 + Opus)
    if not VIDEO_TARGETS["webm"].exists():
        make_video_with_audio_ffmpeg(MASTER_WAV, VIDEO_TARGETS["webm"],
                                    vcodec="libvpx-vp9", acodec="libopus",
                                    size="1280x720", fps=24, bg_color="black",
                                    extra=["-b:v","0","-crf","32"])

    # --- Summary of generated media ---
    created = {k: str(p) for k,p in {**AUDIO_TARGETS, **VIDEO_TARGETS}.items() if p.exists()}
    print("Created media:")
