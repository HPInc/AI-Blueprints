...
from core.generate_metadata.repository_status_tracker import (
...
class CodeGenerationService(BaseGenerativeService):
    ...
    def initialize_embedding_function(self, embedding_model_path=None):
        ...
        model_name = (
            embedding_model_path if embedding_model_path else "all-MiniLM-L6-v2"
        )
        ...
        logger.info(
            f"Successfully initialized HuggingFaceEmbeddings with model: {model_name}"
        )
        ...
        logger.info(
            "ChromaEmbeddingAdapter successfully initialized with all required methods"
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    libcuda_check = subprocess.check_output(
            ...
            logger.info(
                f"CUDA availability check: {'Available' if cuda_available else 'Not available'}"
            )

            # Configure GPU layers based on CUDA availability
            n_gpu_layers = 30 if cuda_available else 0
            logger.info(
                f"  - n_gpu_layers: {n_gpu_layers}, n_batch: 512, n_ctx: {context_window}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Using local LlamaCpp model for code generation with {'GPU' if cuda_available else 'CPU'} mode."
                )
    ...
    def log_model(
        ...
        input_schema = Schema(
            [
                ...
                ColSpec(
                    "string", "repository_url", required=False
        ...
        if embedding_model_path and os.path.exists(embedding_model_path):
            ...
            logger.warning(
                "No local embedding model path provided or path doesn't exist. "
        ...
        mlflow.pyfunc.log_model(
            ...
            python_model=cls(
                delay_async_init=delay_async_init
            ),  # Create instance with delayed initialization
            artifacts=artifacts,
            signature=signature,
            code_paths=["./core", "../src"],
    ...
    def load_context(self, context) -> None:
        ...
        if "embedding_model" in context.artifacts:
            ...
            if os.path.exists(embedding_model_path):
                logger.info(
                    f"Found saved embedding model in artifacts: {embedding_model_path}"
                )
            else:
                logger.warning(
                    f"Embedding model path provided in artifacts but not found: {embedding_model_path}"
                )
                embedding_model_path = None
        ...
        try:
            ...
            logger.warning(
                "Will attempt to initialize default embedding model as fallback"
            )
            try:
