...
class Model:
    ...
    def _resolve_nemo_models(self) -> dict:
        ...
        if self.docs_path:
            ...
            if os.path.exists(models_subdir):
                ...
                if all(
                    os.path.exists(path) for path in models_artifact_models.values()
                ):
                    logger.info(
                        "Using NeMo models from MLflow artifacts/models subdirectory"
                    )
    ...
    def _load_models(self) -> None:
        ...
        try:
            ...
            for model_name, model_path in self.nemo_models.items():
                if not os.path.exists(model_path):
                    raise FileNotFoundError(
                        f"Required NeMo model file not found: {model_path}"
                    )
            ...
            self.asr_model = nemo_asr.models.EncDecCTCModel.restore_from(
                self.nemo_models["enc_dec_CTC"]
            )
            self.spectrogram_generator = nemo_tts.models.FastPitchModel.restore_from(
                self.nemo_models["fast_pitch"]
            )
            self.vocoder = nemo_tts.models.HifiGanModel.restore_from(
                self.nemo_models["hifi_gan"]
            )
    ...
    def translate_text(self, source_text: str) -> str:
        ...
        try:
            ...
            translated_text = self.mt_tokenizer.decode(
    ...
    def text_to_audio(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            spectrogram = self.spectrogram_generator.cuda().generate_spectrogram(
                tokens=parsed_tokens, speaker=2
            )
            audio_tensor = self.vocoder.cuda().convert_spectrogram_to_audio(
                spec=spectrogram
            )
    ...
    def serialize_audio(self, audio_array: np.ndarray, file_id: str) -> str:
        ...
        try:
            ...
            soundfile.write(
                wave_path, audio_array, samplerate=self.framerate, format="WAV"
            )

            # Create base64 encoded version
            with io.BytesIO() as buffer:
                soundfile.write(
                    buffer, audio_array, samplerate=self.framerate, format="WAV"
                )
                buffer.seek(0)
    ...
    def get_onnx_export_config(self) -> List:
        ...
        try:
            ...
            model_configs = [
                ...
                ModelExportConfig(
                    model=self.spectrogram_generator.to(
                        device
                    ),  # 🚀 Pre-loaded NeMo TTS model!
