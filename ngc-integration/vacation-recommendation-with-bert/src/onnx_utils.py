...
class ModelExportConfig:
    ...
    create_triton_structure: bool = (
        False  # Create Triton-style directories or just save ONNX file
    )
    ...
    def validate(self) -> None:
        ...
        if self.model_type in ["pytorch", "sklearn"] and self.input_sample is None:
            raise ValueError(
                f"Input sample required for {self.model_type} model '{self.model_name}'."
            )

        if (
            self.model_type == "tensorflow"
            and self.input_shape is None
            and self.input_sample is None
        ):
            raise ValueError(
                f"Input shape or input sample required for TensorFlow model '{self.model_name}'."
            )
...
def create_model_export_configs(
    ...
    for model_key, model_obj in models_dict.items():
        ...
        config = ModelExportConfig(
            ...
            task=(
                tasks.get(model_key, "text-classification")
                if tasks
...
def _generate_onnx_from_models(
    model_configs: List[ModelExportConfig],
...
def _create_model_directories(
    model_dirs: Union[str, Dict[str, str]], create_triton_structure: bool = False
) -> Dict[str, str]:
    """
    ...
    if create_triton_structure:
        logger.info(
            f"  Creating Triton model directories for {len(models_to_process)} models: {list(models_to_process.keys())}"
        )
    else:
        logger.info(
            f"  Using model directories for {len(models_to_process)} models: {list(models_to_process.keys())}"
        )
        ...
        try:
            if create_triton_structure:
                ...
                if source_path.is_dir():
                    logger.info(
                        f"📁 Copying all contents from {source_dir} to {version_dir}"
                    )
                    ...
                    if not os.path.exists(model_onnx_path):
                        ...
                        onnx_files = [
                            f for f in os.listdir(version_dir) if f.endswith(".onnx")
                        ]
                        if onnx_files:
                            ...
                            logger.warning(
                                f"⚠️ No ONNX file found in directory {source_dir}"
                            )

                    logger.info(
                        f"� Created Triton model directory: {target_model_dir}/1/"
                    )
...
def log_model(
    ...
    **kwargs,
    ...
    try:
        ...
        raise ImportError(
            "MLflow is required but not installed. Install with: pip install mlflow"
        )
    ...
    if models_to_convert_onnx:
        ...
        model_result = _generate_onnx_from_models(model_configs=models_to_convert_onnx)
        ...
        if model_result:
            if isinstance(model_result, dict):
                ...
                for model_name, model_dir in model_result.items():
                    ...
                    logger.info(
                        f"📦 Added model directory artifact: {artifact_key} -> {model_dir}"
                    )
            elif isinstance(model_result, str):
                # Single model directory
                final_artifacts["model_directory"] = model_result
                logger.info(
                    f"📦 Added model directory artifact: model_directory -> {model_result}"
                )
        ...
        if model_result:
            ...
            try:
                ...
                create_triton = any(
                    config.create_triton_structure for config in models_to_convert_onnx
                )

                if create_triton:
                    triton_paths = _create_model_directories(
                        model_dirs=model_result, create_triton_structure=True
                    )
                    ...
                    if triton_paths:
                        for model_name, triton_path in triton_paths.items():
                            ...
                            logger.info(
                                f"📦 Added Triton model artifact: {artifact_key} -> {triton_path}"
                            )

                        logger.info(
                            f"✅ Created {len(triton_paths)} Triton model directories!"
                        )
                else:
                    logger.info(
                        "  No Triton structure requested, using model directories as-is"
                    )
    ...
    try:
        ...
        if models_to_convert_onnx and model_result:
            if isinstance(model_result, dict):
                logger.info(
                    f"✅ Model logged with {len(model_result)} model directories created!"
                )
            else:
...
def create_model_directories_standalone(
    model_sources: Union[str, Dict[str, str], List[str]],
    ...
    if create_triton_structure:
        logger.info(
            f"  Creating Triton model directories for {len(models_dict)} models"
        )
    else:
    ...
    try:
        return _create_model_directories(
            model_dirs=models_dict, create_triton_structure=create_triton_structure
        )
    finally:
