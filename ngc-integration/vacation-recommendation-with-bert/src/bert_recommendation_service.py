...
class BERTModelWithHiddenStates(
    torch.nn.Module
):  # Pytorch models that use **kwargs needs to create wrapper, we need more work on that
...
class BERTTourismModel(mlflow.pyfunc.PythonModel):
    ...
    def load_context(self, context):
        ...
        self.bert_model = BERTLMModel.restore_from(
            context.artifacts["bert_model_path"], strict=False
        ).to(self.device)
    ...
    def log_model(
        ...
        bert_model = BERTLMModel.restore_from(
            bert_model_datafabric_path, strict=False
        ).to(device)
        bert_model.eval()

        wrapped_model = BERTModelWithHiddenStates(
            bert_model
        )  # it doesn't have oficial nemo export function so its necessary to recreate the model as torch to use torch conversion
